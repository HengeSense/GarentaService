//
//  DamageManagementVCViewController.m
//  GarentaService
//
//  Created by Alp Keser on 10/15/13.
...
@interface DamageManagementVCViewController ()

@end

@implementation DamageManagementVCViewController
...
- (void)viewDidLoad
{
	...
	CC3Layer* cc3Layer = [GarentaServiceLayer layerWithController: _viewController];
	
	// Create the customized 3D scene and attach it to the layer.
	// Could also just create this inside the customer layer.
	cc3Layer.cc3Scene = [GarentaServiceScene scene];
...
-(void) establishDirectorController {
	...
	//_viewController = [CC3DeviceCameraOverlayUIViewController new];
	_viewController = (CC3DeviceCameraOverlayUIViewController*)CC3DeviceCameraOverlayUIViewController.sharedDirector;
	_viewController.supportedInterfaceOrientations = UIInterfaceOrientationMaskAll;
	_viewController.viewShouldUseStencilBuffer = NO;		// Set to YES if using shadow volumes
	_viewController.viewPixelSamples = 1;					// Set to 4 for antialiasing multisampling
	_viewController.animationInterval = (1.0f / 60);
